...
export class GateFlowAgent {
    ...
    executors;
    orchestrator = null;
    constructor(bus, toolContext, config) {
        this.bus = bus;
        this.config = {
    ...
    async run(userMessage, options) {
        ...
        this.session.thinkingChain.addAnalysisStep(`Analyzing request in ${mode} mode`, { mode, userMessage }, 0.9);
...
Return needsMultiAgent: true only for genuinely complex requests.`
    ...
    summarizeArgs(args) {
        ...
        if ('module' in obj)
            return String(obj.module);
        if ('pattern' in obj)
            return String(obj.pattern);
        if ('top' in obj)
            return String(obj.top);
    ...
    summarizeResult(result) {
        ...
        if ('replacements' in obj)
            return `${obj.replacements} replacements`;
        if ('totalMatches' in obj)
            return `${obj.totalMatches} matches`;
