...
import { EventBus, ExitCodes } from '../events/index.js';
import { initPolicyEngine } from '../approval/index.js';
import { createTools } from '../fileops/index.js';
import { DiffEngine } from '../diff/index.js';
import { ProjectIndexer } from '../indexer/index.js';
import { GateFlowAgent } from '../agent/index.js';
...
// Chat Command (REPL)
// ============================================================================
export async function chatCommand(ctx, initialQuery) {
    // Build tool context
    const toolContext = {
        bus: ctx.bus,
        policy: ctx.policy,
...
// Scan Command
// ============================================================================
export async function scanCommand(ctx) {
    try {
...
// Lint Command
// ============================================================================
export async function lintCommand(ctx, files) {
...
// Fix Command
// ============================================================================
export async function fixCommand(ctx, file) {
    // Build tool context
    const toolContext = {
        bus: ctx.bus,
        policy: ctx.policy,
...
// Watch Command
// ============================================================================
export async function watchCommand(ctx, patterns) {
    const watchConfig = patterns.length > 0 ? { patterns } : undefined;
    const watcher = new WatchManager(ctx.projectRoot, ctx.bus, ctx.indexer, ctx.verilator, watchConfig);
    watcher.start();
...
// Generate Command
// ============================================================================
export async function generateCommand(ctx, type, name, options) {
    // Build tool context
    const toolContext = {
        bus: ctx.bus,
        policy: ctx.policy,
