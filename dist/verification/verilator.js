...
export class Verilator {
    ...
    async lint(filePath, options) {
        ...
        try {
            ...
            const fullCmd = this.useWsl
                ...
                : `${binaryCmd} ${args.join(' ')}`;
            const { stdout, stderr } = await execAsync(fullCmd, { timeout: this.config.timeout });
     ...
     */
    async compile(files, topModule, options) {
        const startTime = Date.now();
        ...
        try {
            const { stdout, stderr } = await execAsync(`${this.config.binary} ${args.join(' ')}`, {
     ...
     * Run simulation
     */
    async simulate(topModule, options) {
        const startTime = Date.now();
