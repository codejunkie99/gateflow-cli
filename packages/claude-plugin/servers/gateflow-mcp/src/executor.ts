...
export function createToolExecutor(projectRoot: string): ToolExecutor {
    ...
    const executors: Record<string, (args: Record<string, unknown>) => Promise<unknown>> = {
        ...
        async read_file(args) {
            ...
            const start = (args.startLine as number | undefined) ?? 1;
            const end = (args.endLine as number | undefined) ?? lines.length;
        ...
        async search_replace(args) {
            ...
            const search = args.isRegex
                ? new RegExp(args.search as string, args.all ? 'g' : '')
                : args.search as string;

            const before = content;
            if (args.all) {
                content = content.split(search as string).join(args.replace as string);
            } else {
        ...
        async list_files(args) {
            const directory = resolvePath(args.directory as string);
            const extensions = (args.extensions as string[]) ?? ['.sv'];
        ...
        async search_code(args) {
            ...
            const filePattern = (args.filePattern as string) ?? '**/*.sv';
        ...
        async run_simulation(args) {
            const top = args.top as string;
            const testbench = args.testbench ? resolvePath(args.testbench as string) : undefined;
        ...
        async analyze_waveform(args) {
            const vcdPath = resolvePath(args.vcdPath as string);
        ...
        async check_tool_status(args) {
            const tool = (args.tool as string) ?? 'all';
            const status: Record<string, unknown> = {};

            const checkCommand = (cmd: string): { installed: boolean; version?: string; path?: string } => {
                try {
        ...
        async plan_complex_task(args) {
            ...
            const suggestedSteps: Array<{ step: number; agent: string; task: string; parallel?: boolean }> = [];
            let stepNum = 1;
        ...
        async parallel_agents(args) {
            const tasks = args.tasks as Array<{ agent: string; task: string; files?: string[] }>;
        ...
        async get_file_chunk(args) {
            const filePath = resolvePath(args.path as string);
            const chunkType = args.chunkType as string;
            const content = await fs.readFile(filePath, 'utf-8');
