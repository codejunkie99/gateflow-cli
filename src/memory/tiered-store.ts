...
import type { KnowledgeItem } from "./ParentKnowledgeStore.js";
...
export class TieredKnowledgeStore {
  ...
  markAccessed(id: string): void {
    ...
    if (
      meta.accessCount >= this.config.warmThreshold &&
      !this.hotItems.has(id)
    ) {
      const item = this.itemLookup?.(id);
      if (item) {
        this.promoteToHot(id, item);
  ...
  rebalance(): void {
    ...
    for (const [id, meta] of this.metadata) {
      ...
      const daysSinceAccess = Math.max(
        0,
        (now - meta.lastAccessed) / (24 * 60 * 60 * 1000),
      );
      const score = item
  ...
  private calculateRelevanceScore(item: KnowledgeItem, now: number): number {
    ...
    const daysSinceAccess = Math.max(
      0,
      (now - item.lastAccessed) / (24 * 60 * 60 * 1000),
    );
  ...
  private demoteColdest(): void {
    ...
    for (const [id, item] of this.hotItems) {
      ...
      const score =
        this.calculateRelevanceScore(item, now) + (meta?.accessCount ?? 0) * 5;
