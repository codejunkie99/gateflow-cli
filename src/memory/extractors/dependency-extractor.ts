 ...
 */
function groupByReason(deps: FileDependency[]): Map<string, FileDependency[]> {
...
function formatContent(
  ...
  switch (dep.reason) {
    case "instantiates":
      lines.push(
        `${fromBase} instantiates module ${dep.entityName} from ${toBase}`,
      );
      break;
    case "imports":
      lines.push(
        `${fromBase} imports package ${dep.entityName} from ${toBase}`,
      );
      break;
    case "includes":
    ...
    case "uses_macro":
      lines.push(
        `${fromBase} uses macro ${dep.entityName} defined in ${toBase}`,
      );
      break;
    default:
...
function buildTags(dep: FileDependency): string[] {
  const tags = ["dependency", dep.reason, "namespace:facts"];
...
function shouldIncludeFiles(
  ...
  if (options.excludePatterns?.length) {
    for (const pattern of options.excludePatterns) {
      if (
        picomatch.isMatch(fromFile, pattern) ||
