...
export function extractModuleInfo(
  ...
  const moduleDeclarations = project.declarations.filter((decl) =>
    ["module", "interface", "package"].includes(decl.kind),
  );
  ...
  for (const decl of moduleDeclarations) {
    ...
    const params = findChildParameters(
      decl.id,
      decl.name,
    ...
    const isTop = !project.instances.some(
      (inst) => inst.targetName === decl.name || inst.resolvedId === decl.id,
    );
...
function formatPortSummary(ports: Declaration[]): string {
  ...
  if (portStrings.length > 10) {
    return (
      portStrings.slice(0, 10).join(", ") + `, ... (${ports.length - 10} more)`
    );
  }
...
function formatParamSummary(params: Declaration[]): string {
  ...
  if (paramStrings.length > 5) {
    return (
      paramStrings.slice(0, 5).join(", ") + `, ... (${params.length - 5} more)`
    );
  }
...
function buildKeywords(
  ...
  for (const p of ports.slice(0, 20)) {
    // Limit to avoid huge keyword lists
