...
import type { KnowledgeStore } from "../memory/ParentKnowledgeStore.js";
...
export class FixLoop {
  ...
  async init(filePath: string): Promise<void> {
    if (this.knowledgeStore) {
      try {
        const loaded = await this.loadHistoricalPatterns(
          this.knowledgeStore,
          filePath,
        );
        if (loaded > 0) {
  ...
  async run(filePath: string): Promise<FixLoopResult> {
    ...
    while (
      ...
      const fileContent =
        fileResult.success && fileResult.content
          ? fileResult.content
          : "(Could not read file)";
  ...
  private checkThrashing(errors: LintError[]): {
    ...
    for (const sig of signatures) {
      ...
      const sessionAttempts = this.currentSession.filter(
        (a) => a.errorSignature === sig,
      );
...
Please try a DIFFERENT approach than previous attempts.`;
  ...
  private formatAttemptContent(
    ...
    if (successfulAttempts.length > 0) {
      lines.push(
        `Successful fix (${successfulAttempts.length}/${allAttempts.length} attempts):`,
      );
      lines.push("```");
      ...
      lines.push(
        `No successful fix found after ${allAttempts.length} attempts.`,
      );
      lines.push("");
    ...
    lines.push("");
    lines.push(
      `Attempt history: ${allAttempts.length} total, ${successfulAttempts.length} successful`,
    );
  ...
  private truncateFix(fix: string): string {
    ...
    return (
      fix.slice(0, maxLength - 50) +
      "\n... (truncated, " +
      (fix.length - maxLength + 50) +
      " more chars)"
    );
  }
