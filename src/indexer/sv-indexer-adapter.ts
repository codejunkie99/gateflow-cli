...
import { getMemoryService } from "../memory/MemoryService.js";
...
export class SVIndexerAdapter {
  ...
  async buildIndex(options?: {
    ...
    if (!options?.skipCache) {
      const cachedProject = await this.projectCache.get(
        this.rootPath,
        this.filePaths,
      );
      if (cachedProject) {
  ...
  searchModules(pattern: string): ModuleInfo[] {
    ...
    return Array.from(this.legacyIndex.modules.values()).filter((m) =>
      regex.test(m.name),
    );
  }
  ...
  private async extractKnowledge(): Promise<void> {
    ...
    try {
      ...
      const options = createExtractionOptions(knowledgeStore.getProjectId(), {
        maxItemsPerCategory: 300, // Reasonable limit for typical projects
        defineContextId: this.project?.defineContextId,
        compileOrderId: this.project?.compileOrderId,
      });

      if (this.project?.defineContextId) {
        // Use MemoryService if available (keeps both layers in sync),
        // otherwise fall back to KnowledgeStore directly
        const memoryService = getMemoryService();
        if (memoryService) {
          memoryService.setActiveContext(
    ...
    for (const fileRecord of this.project.files) {
      ...
      for (const decl of decls) {
        if (decl.kind === "module") {
          ...
          this.legacyIndex.modules.set(
            decl.name,
            this.convertModule(decl, filePath, decls),
          );
          ...
          this.legacyIndex.interfaces.set(
            decl.name,
            this.convertInterface(decl, filePath, decls),
          );
        }
      }

      // Extract imports from references
      const fileRefs = this.project.references.filter(
        (r) => r.location.file === filePath,
      );
      ...
      const fileDirectives = this.project.directives.filter(
  ...
  private convertModule(
    ...
    for (const decl of allDecls) {
      if (
        decl.scope.length > 0 &&
        decl.scope[decl.scope.length - 1] === moduleDecl.name
      ) {
        if (decl.kind === "port" && decl.data.kind === "port") {
          ports.push({
            name: decl.name,
            direction:
              decl.data.direction === "ref" ? "inout" : decl.data.direction,
    ...
    if (this.project) {
      const moduleInstances = this.project.instances.filter(
        (inst) =>
          ...
          inst.parentScope[inst.parentScope.length - 1] === moduleDecl.name,
      );
  ...
  private convertInterface(
    ...
    for (const decl of allDecls) {
      if (
        decl.kind === "modport" &&
        decl.scope.length > 0 &&
        decl.scope[decl.scope.length - 1] === ifaceDecl.name
      ) {
