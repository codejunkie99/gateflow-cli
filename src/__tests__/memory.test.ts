...
import { KnowledgeStore, type KnowledgeItem } from '../memory/KnowledgeStore.js';
...
describe('Memory Module', () => {
    ...
    describe('MemoryManager', () => {
        ...
        it('should maintain strict history limits', async () => {
            ...
            manager.addConversation({ summary: '1', timestamp: 1, filesModified: [], exitCode: 0 });
            manager.addConversation({ summary: '2', timestamp: 2, filesModified: [], exitCode: 0 });
            manager.addConversation({ summary: '3', timestamp: 3, filesModified: [], exitCode: 0 });
    ...
    describe('KnowledgeStore', () => {
        ...
        describe('Knowledge Extraction', () => {
            it('should extract patterns from lint sessions', async () => {
                ...
                const extracted = store.extractFromLintSession('session-1', errors, fixes);
    ...
    describe('Token Estimator', () => {
        describe('content detection', () => {
            ...
            it('should detect natural language', () => {
                const text = 'This function implements a queue for data buffering and processing.';
            ...
            it('should detect mixed content', () => {
                ...
                const mixed = 'The module uses a wire connection and has a clk input signal for timing.';
    ...
    describe('Tiered Knowledge Store', () => {
        function createTestItem(id: string, useCount = 0, daysOld = 0): KnowledgeItem {
            const now = Date.now();
            return {
                id,
                fingerprint: `fp-${id}`,
        ...
        it('should initialize with items', () => {
            ...
            const items = Array.from({ length: 10 }, (_, i) => createTestItem(`item-${i}`));
        ...
        it('should promote items on repeated access', () => {
            ...
            const items = Array.from({ length: 5 }, (_, i) =>
                createTestItem(`item-${i}`, 5, 5)  // Same useCount=5, daysOld=5
            );
            const itemMap = new Map(items.map(i => [i.id, i]));
